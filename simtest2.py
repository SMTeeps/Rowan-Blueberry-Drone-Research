...
async def run():
    ...
    await drone.offboard.set_position_ned(PositionNedYaw(0.0, 0.0, 0.0, angle))
    ...
    async for pos in drone.telemetry.position_velocity_ned():
        if int(pos.position.north_m) == ds.getY() and int(pos.position.east_m) == ds.getX():
            ...
            angle = 90 - math.degrees(math.atan2(ds.getY() - pos.position.north_m, ds.getX() - pos.position.east_m))
            print(f"-- Turn to face {ds.getY()}N, {ds.getX()}E")    
            await drone.offboard.set_position_ned(PositionNedYaw(pos.position.north_m, pos.position.east_m, -25.0, angle))
            await asyncio.sleep(10)
            print("-- Start moving 2m/s forward")
            await drone.offboard.set_velocity_body(VelocityBodyYawspeed(2.0, 0.0, 0.0, 0.0))
            break
